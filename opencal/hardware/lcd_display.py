...
class LCDDisplay:
    def __init__(self, config: LcdDisplayConfig):
        ...
        self.address = int(config.address, 16)  # Address is a hex string
        ...
        # Event to terminate scrolling at any point. Necessary when LCD updates midway through scroll loop.
        self.refresh_scroll_event = threading.Event()
    ...
    def write_message(self, message: str, row: int = 0, _col: int = 0):
        ...
        if len(message) > self.cols:
            ...
            # Update the framebuffer with the new message
            self.framebuffer[row] = message
            self.refresh_scroll_event.set()
    ...
    def _scrolling_loop(self):
        ...
        # TODO: Scrolling logic can be refactored and simplified greatly I think
        while self.scrolling_active:
            ...
            for row, text in scrolling_items:
                ...
                for i in range(len(text) - self.cols + 1):
                    # Do not finish scrolling to the end of the line if a signal was sent that the scrolling items changed
                    if self.refresh_scroll_event.is_set():
                        self.refresh_scroll_event.clear()
                        break
                    # Update the framebuffer with the scrolling text
                    self.framebuffer[row] = text[i : i + self.cols]
                    self._update_lcd(row)  # Only update the scrolling row
                    sleep(self.scroll_interval)  # Adjust the speed of scrolling
...
if __name__ == "__main__":
    ...
    lcd_display.write_message(
        "This is a long scrolling message on row 1!", row=1
    )  # Write a long message for scrolling
    lcd_display.write_message("Static Line 3", row=2)  # Write another static message
    lcd_display.write_message(
        "This is another long scrolling text for row 3.", row=3
    )  # Another long message for scrolling
